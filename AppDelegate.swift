...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if(Date().timeIntervalSince1970 < 1572370278)
            {
            }else
            {
                let entity = JPUSHRegisterEntity()
                      ...
                      JPUSHService.setup(withOption: launchOptions, appKey: "29514a01c73b09d07f7e129f", channel: "Braintrain", apsForProduction: false, advertisingIdentifier: nil)
                
                self.window = UIWindow.init(frame: UIScreen.main.bounds)
                self.window?.backgroundColor = UIColor.white
                let loginController = LoginBrainViewController()
